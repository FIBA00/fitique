...
const preferenceOptions = [
	...
	},
	{
		id: "push",
		title: "Push alerts",
		detail: "In-app and device alerts while Fitique is available on your device.",
		icon: BellRing,
	},
];
		...
		<section className="border border-fitique-line bg-fitique-ivory p-5">
			<div className="flex items-start justify-between gap-3">
				<div className="flex gap-3">
					<BellRing
						size={21}
						className="mt-0.5 shrink-0 text-fitique-plum"
					/>
					<div>
			...
			<p className="mt-3 text-xs leading-5 text-fitique-ink/65">
				Choose the updates that feel helpful. Your selections stay
				linked to this delivery and will sync as soon as your messaging
				service is ready.
			</p>
			<div className="mt-5 divide-y divide-fitique-line border-y border-fitique-line">
				{preferenceOptions.map(({ id, title, detail, icon: Icon }) => (
					<div key={id} className="flex items-center gap-3 py-3">
						<Icon
							size={17}
							className="shrink-0 text-fitique-brown"
						/>
			...
			<div className="mt-6 border-t border-fitique-line pt-5">
				...
				{recordQuery.isLoading ? (
					...
					<p className="mt-2 text-xs leading-5 text-fitique-ink/60">
						Your carrier and tracking reference will appear here
			...
			<div className="mt-6">
				...
				{eventsQuery.isLoading ? (
						...
						{events.map((event, index) => (
							<li
								...
								<span className="min-w-0 pt-0.5">
									<strong className="text-fitique-ink">
										{event.status}
									</strong>
					...
					<p className="mt-3 text-xs leading-5 text-fitique-ink/60">
						Your delivery story will begin here when its first
						update arrives.
					</p>
				)}
			</div>
			...
			<p className="mt-5 border-l-2 border-fitique-brown bg-fitique-paper px-3 py-2 text-[.68rem] leading-5 text-fitique-ink/60">
				<MessageSquare
					className="mr-1 inline text-fitique-brown"
					size={13}
				/>{" "}
				Every alert choice, delivery time, tracking reference, and
				carrier event is ready to receive live data from your services.
			</p>
		</section>
	);
}
